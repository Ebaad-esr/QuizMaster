...
// ** NEW: Handle Next Question Button Click **
...
socket.on("quizState", state => {
    ui.quizTitle.textContent = state.quizName || "QuizCraft";
});
...
socket.on("question", ({ question, index }) => {
    // Hide feedback and next button at start of new question
    ...
    JSON.parse(question.options).forEach((optionText, optionIndex) => {
        ...
        // New QuizCraft Theme classes for options
    ...
    timerInterval = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            clearInterval(timerInterval);
            // If time runs out and not answered, submit null
            if (!ui.optionsContainer.querySelector('button:disabled')) {
                submitAnswer(null);
            }
            // Note: We removed the automatic setTimeout here. 
            // It will now wait for the user to click the button.
        }
    }, 1000);
});
...
socket.on("answerResult", ({ isCorrect, scoreChange, correctOptionIndex, selectedOptionIndex, score }) => {
    ...
    // Focus on the next button for accessibility/speed
    ...
    buttons.forEach((button, index) => {
        if (index === correctOptionIndex) {
            button.style.backgroundColor = "rgba(22, 163, 74, 0.8)"; // Green
            button.style.borderColor = "#22c55e";
        } else if (index === selectedOptionIndex) {
            button.style.backgroundColor = "rgba(220, 38, 38, 0.8)"; // Red
